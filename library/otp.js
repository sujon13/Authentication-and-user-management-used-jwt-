    ...
    create = (email) => {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if(exist)console.log(`Previous entry deleted! it was: ${exist}`);

                // Now save it to database
                var savedOtp = await otp.save();
                if(!savedOtp) return reject({ statusCode: 500, error: 'Internal server error'});
                ...
                if(!sentMail) return reject({ statusCode: 500, error: 'Internal server error'});
    ...
    verify = (id, otp) => {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if(!document)return reject({ statusCode: 401, error: 'OTP is expired! Please resend'});

                const elapsedTime = Math.floor((Date.now() - document.createdAt)/1000);
                if(elapsedTime > 60)return reject({ statusCode: 401, error: 'OTP is expired! Please resend'});

                if(document.otp !== otp)return reject({ statusCode: 400, error: 'Invalid otp'});
            } catch(error) {
                console.log(error);
                return reject({ statusCode: 500, error: 'Internal server error'});
            }
